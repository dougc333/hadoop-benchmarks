...
function moddaemon {
  ...
  if [ -e tmpdaemon/yarn-daemon.shorig ];then
...
function modsite {
  ...
  if [ -e tmpsite/mapred-site.xmlorig ];then 
   sudo sed "s/var/\/testhdfsvol/g" tmpsite/mapred-site.xmlorig > tmpsite/mapred-site.xml
  fi
  ...
  if [ -e tmpsite/yarn-site.xmlorig ]; then 
     sed "s/var/\/testhdfsvol/g" tmpsite/yarn-site.xmlorig > tmpsite/yarn-site.xml
  fi
